...
fb += '\n'.join([
    ...
    lead('Flock Bio sells pooled DNA libraries to scientists running CRISPR screens, protein and enzyme engineering, MPRAs and CAR-T work.'),
    prose('There is no job title that reliably identifies that person. Filtering LinkedIn by &ldquo;Scientist&rdquo; at biotech companies returns tens of thousands of people, almost none of whom are building a library this quarter.',
          'What they had instead was a list of the phrases their buyers actually use. That turned out to be a better targeting signal than any firmographic filter.'),
    ...
    bul([
        '<b>The buyers are technical.</b> A generic message marks you instantly as someone who does not understand the work, which is disqualifying in a field where credibility is the product.',
        '<b>LinkedIn punishes volume.</b> Connection limits and spam signals mean the channel itself constrains how much you can send.',
        '<b>Every message has to be worth receiving the first time,</b> because there is no second list.',
    ]),
    ...
    prose('Someone who comments on a post about directed evolution has self-selected far more precisely than any title filter could. Two n8n workflows do the work, plus a third that exists only to fail safely.'),

    layer('Workflow 1', 'Sourcing and qualification'),
    prose('An Apify LinkedIn post scraper runs every three days against 19 quoted phrases &mdash; <code>"gene library"</code>, <code>"codon optimization"</code>, <code>"directed evolution"</code>, <code>"AAV cassette design"</code>, <code>"gRNA libraries"</code> and the rest &mdash; pulling the past week&rsquo;s posts <b>along with their comments and reactions</b>.',
          'Then two stages of filtering, mechanical before semantic.'),
    bul([
        '<b>Mechanical.</b> Keep posts by people, drop company pages. Later, drop any profile URL containing <code>/company/</code>.',
        '<b>Semantic.</b> An LLM reads each post and returns true or false on whether it is genuine industry discussion, rejecting personal stories and webinar pitches with a one-line reason.',
        '<b>Re-join.</b> The model judges <em>posts</em>, but the leads are the <em>engagers</em>. A code step takes the approved post IDs and pulls back every comment and reaction attached to them.',
    ]),
    ...
    prose('Approved leads flow from a Google Sheet into HeyReach every three days. An LLM first cleans the name, stripping titles, suffixes, emojis and pipes down to a clean first and last name, because &ldquo;Dr. WALLIS MARGRAFF, Ph.D.&rdquo; is not how you open a message.'),
    prose('A switch then routes each lead down one of two tracks, and this is where the personalisation actually lives:'),
    bul([
    ...
    notes(['<b>65 connections sent, 51 accepted, 47 messages, 20 replies.</b> Deliberately low volume: the constraint was never how many could be sent.',
           '<b>Cold LinkedIn benchmarks sit in the low single digits.</b> A 42.6% reply rate is an order of magnitude above the channel, and it came from targeting quality rather than send volume.',
           '<b>Replies turned into booked meetings,</b> which is the number that actually matters to them.']),
    pending(['HeyReach KPI screenshot goes here',
             'save it to /shots/flock-metrics.png and I will place it']),

    sec_id('05', 'What it taught me', 'Intent beats firmographics', 'learned'),
    lead('The instinct in outbound is to describe the buyer and go find people who match the description. On a narrow technical list that fails, because the description does not distinguish the person building a library this quarter from the ten thousand who are not.'),
    prose('What someone posted about last week does distinguish them. Targeting the conversation rather than the person is the single decision that produced the reply rate.',
          'The rest is discipline: enrich properly, dedupe by intent rather than by row, never contact anyone twice, and make the system stop itself when it breaks.'),
