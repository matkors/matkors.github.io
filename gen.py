...
hc = HEAD.format(
    ...
    desc='3 voice agents built for a national health insurance marketplace, where the agent is not allowed to improvise.',
...
hc += '\n'.join([
    ...
    seealso('Both of the other case studies have the screenshots, the workflows and the numbers.',
            [('World of Luxury', '6 live n8n workflows, real call data', '/work/world-of-luxury/'),
    ...
    sec_id('03', 'What I built', '3 agents, one guardrail layer', 'built'),
    bul([
    ...
    prose('I scoped all 3 from scratch: sat with non-technical executives, mapped the use cases, designed the flows, presented them back in plain language, and coordinated across product, engineering, compliance and operations to get them live. Nobody handed me a spec.'),
    ...
    bul([
        '<b>Write the adversarial cases first.</b> &ldquo;So will my insulin be covered?&rdquo; in 6 phrasings, the polite ones and the desperate ones. If the agent breaks, it breaks there.',
...
wol = HEAD.format(
    ...
    desc='A production voice agent for a luxury watch dealer: live Shopify catalog, fuzzy model-number matching at 90-95% across 5,000 products, and 89% of calls arriving after hours.',
...
wol += '\n'.join([
    glance([('Role', 'Built it and run it'),
            ...
            ('Catalog', '~5,000 live products'),
            ('Match rate', '90&ndash;95%')]),
    pull('The voice was never the hard part. The hard part was matching a misheard watch name against 5,000 products that change every day, fast enough that the caller does not notice the pause.'),
    ...
    prose('People shop for a $40,000 watch at 11pm and on Sunday afternoons. Those calls used to hit a voicemail box nobody was going to action before Monday.',
    ...
    sec_id('02', 'The constraint', '5,000 products, changing daily', 'constraint'),
    lead('The agent had to hit live inventory mid-call, and fast. A lookup that takes 8 seconds has already damaged the conversation.'),
    prose('The catalog moves every day as pieces sell and new stock lands, so anything working from a static export is wrong within 24 hours.',
    ...
    bul([
        ...
        '<b>Around 100 products are near-duplicates.</b> Exact match returns nothing; loose match returns the wrong watch, confidently.',
        '<b>Callers ask about orders too,</b> so the agent has to pick which lookup to run.',
    ]),

    sec_id('03', 'What I built', '6 workflows in 3 layers', 'built'),
    ...
    prose('The obvious approach broke immediately. n8n&rsquo;s Shopify node returns 50 records per call, and looping it hard enough to finish a full pull kept tripping limits and deactivating the workflow.',
          'I replaced it with a custom HTTP request walking Shopify&rsquo;s <code>page_info</code> cursor at roughly 250 records per iteration. 5x the throughput, and stable enough to run unattended.',
    ...
    prose('3 lookup workflows sit behind webhooks the agent calls as tools. Each runs the same shape: extract the argument, normalise it, search Airtable, rank the candidates, format for speech.'),
    ...
    fig('Fig 4', 'Lookup by model number', 'lookup-reference.png',
        'Same shape pointed at the reference field. Normalisation does the heavy lifting here, stripping the case and punctuation that make a direct match return nothing. <b>90 to 95% correct across 5,000 products.</b>'),
    ...
    stats([('89%', 'of calls arrive after hours', 1, ('89', '%')),
           ('90&ndash;95%', 'correct product from 5,000 products', 0, None),
           ('8 mo', 'retained, still running', 0, None),
           ('$21&ndash;302K', 'range of pieces asked about', 0, None)]),
    notes(['<b>72% of genuine inquiries</b> become a captured lead, after excluding wrong numbers, a caller chasing a Macy&rsquo;s order, and 2 who were abusive. Counting those would have flattered the number.',
    ...
    prose('3 from the last pass, none of which show up in any metric:'),
    bul([
        ...
        '<b>Lead capture asked too late.</b> Callers answered 3 or 4 product questions, then hung up the moment the agent asked for a name. The ask arrived cold at the end instead of being earned earlier.',
...
fb += '\n'.join([
    ...
    lead('The addressable audience is small enough that burning it has a real cost. There is no version of this where you send 10,000 messages and accept a low hit rate.'),
    bul([
    ...
    prose('Someone who comments on a post about directed evolution has self-selected far more precisely than any title filter can manage. 2 n8n workflows do the work, plus a third that exists only to fail safely.'),

    layer('Workflow 1', 'Sourcing and qualification'),
    prose('An Apify scraper runs every 3 days against 19 quoted phrases such as <code>"gene library"</code>, <code>"codon optimization"</code>, <code>"directed evolution"</code>, <code>"AAV cassette design"</code>, pulling the week&rsquo;s posts <b>along with their comments and reactions</b>.'),
    bul([
    ...
    prose('Approved leads flow from a Google Sheet into HeyReach every 3 days. An LLM first strips titles, suffixes, emojis and pipes down to a clean first and last name, because &ldquo;Dr. WALLIS MARGRAFF, Ph.D.&rdquo; is not how you open a message.'),
    ...
    fig('Fig 3', 'The error workflow', 'flockbio-error.png',
        '3 nodes, and the order of the middle 2 is the entire design. Reverse them and a broken run keeps sending while the email sits unread.'),
    ...
    prose('On a narrow technical list that fails, because the description does not separate the person building a library this quarter from the 10,000 who are not.',
